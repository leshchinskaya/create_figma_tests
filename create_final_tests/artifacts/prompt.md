...
* Для блоков кода (если нужны) *всегда* используй "{code}text{code}" конструкцию, но будь внимателен с фигурными
  скобками
* Для форматирования запросов приоритизируй {{}} формат в виде списка вместо блока кода
...
3. Отправка запроса
    1. Отправка запроса
        ...
        * Запрос в *названии* должен отражать цель запроса, а не конкретный метод (например запрос авторизации,
          запрос смс кода). В шагах же каждый запрос обязательно описан в виде метод + путь, например
          `{{GET /api/endpoint}}`. Несколько запросов формируй в виде буллит списка.
