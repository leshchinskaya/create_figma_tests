...
1. Компоновки
    1. Компоновка экрана
        ...
        * Шаги - каждый шаг проверяет один элемент, где action = элемент. Порядок сверху вниз по экрану.
            ...
            * TabBar - проверить его отображение, указать активный таб, проверить переход на другие табы и обратно
              (состояние должно сохраниться), проверить список табов, при вложенной навигации проверить возврат на
              прошлый или корневой экран (например из категории каталога в каталог).
...
3. Отправка запроса
    1. Отправка запроса
        ...
        * Запрос в *названии* должен отражать цель запроса, а не конкретный метод (например запрос авторизации,
          запрос смс кода).
        * В *шагах* каждый запрос *обязательно* описан в виде метод + путь, например `{{GET /api/endpoint}}`.
    ...
    2. Отправка запроса с ошибкой
        ...
        * Логика совпадает с успешной отправкой запроса со следующими уточнениями:
            ...
            * Для параллельных запросов важно проверить оба варианта (ошибка только 1-го и только 2-го запроса).
              Обычно ошибка любого запроса приводит к обработке, однако внимательно проверяй ТЗ, иногда запросы бывают
              необязательными и обрабатываются по другому. Покрывай эту логику очень аккуратно.
            ...
            * В каждом тесте на ошибку запросов должны проверяться все общие ошибки проекта (400, 500, таймаут, без
              интернета обязательно, так же в общем ТЗ могут быть дополнительные) и специфичные указанные для
              конкретного запроса.
            ...
            * В отличие от HTTP ошибок, без интернета не может случиться между запросами или для одного запроса из
              ...
              результат".
            * Не должно быть одновременно error/no connection state и снеков. Снек подразумевает что содержимое экрана
...
4. Пагинация
    ...
    2. Пагинация с ошибкой
        ...
        * Логика совпадает с отправкой запроса с ошибкой со следующими уточнениями:
            * При ошибке может быть снек или маленький error state в конце списка.
            * При снеке сразу проверяем что можно повторить пагинацию, заглушку проверяем далее отдельными тестами.
    3. Повтор пагинации
...
* Если в документации встречаются противоречия, неполные данные или явные TODO, сделай наиболее вероятное предположение,
  ...
  текстом-иконкой "(?)". После генерации тестов сделай сводку всех моментов в проверках которые вызвали у тебя
  неуверенность в виде пронумерованного списка.
