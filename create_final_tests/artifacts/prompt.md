...
Приложены файлами pdf. Воспринимай требования к общей логике не как отдельную фичу, а как переиспользуемую логику
которую возможно надо проверить в данной фиче.
...
* description - должен содержать название файла ТЗ и ссылку на дизайн. Каждый пункт на отдельной строке
...
* Для блоков кода (если нужны) *всегда* используй "{code}text{code}" конструкцию, но будь внимателен с фигурными
  скобками
* Для запросов приоритизируй {{}} формат в виде списка вместо блока кода
