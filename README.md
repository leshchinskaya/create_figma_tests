...
### 4. Конвертация PDF и Confluence в Markdown (`convert_pdf_to_req.py`)

Этот скрипт предназначен для преобразования документов из PDF (локальных файлов или по URL) и страниц Confluence (включая дочерние страницы и вложения) в структурированный набор Markdown файлов. Это облегчает извлечение и использование текстового содержимого и связанных медиафайлов для дальнейшего анализа или создания тестовых артефактов.

**Основные Возможности:**

*   **Обработка PDF:**
    *   Принимает путь к локальному PDF-файлу или URL, указывающий на PDF.
    *   Извлекает текст постранично.
    *   Сохраняет текст каждой страницы в отдельный `content.md` файл.
*   **Обработка Confluence:**
    *   Принимает URL страницы Confluence.
    *   Загружает HTML-содержимое страницы и всех ее дочерних страниц.
    *   Конвертирует HTML в Markdown, сохраняя основное форматирование.
    *   Автоматически скачивает все вложения (изображения, файлы), найденные на странице Confluence.
    *   Сохраняет вложения в подпапку `attachments`.
    *   Обновляет ссылки на вложения в Markdown файлах, чтобы они указывали на локально скачанные файлы.
*   **Структура Вывода:**
    *   Все обработанные документы сохраняются в базовую директорию `create_final_tests/folder_structure/`.
    *   Для каждого исходного документа (PDF или страница Confluence) создается уникальная папка `имя_документа`.
        *   **PDF:** `create_final_tests/folder_structure/имя_документа_pdf/page_N/content.md`
        *   **Confluence:** `create_final_tests/folder_structure/confluence_ID_страницы/page_1/content.md` (для основного содержимого страницы) и `create_final_tests/folder_structure/confluence_ID_страницы/page_1/attachments/` (для вложений).

**Конфигурация (в `config.py`):**

*   Для работы с Confluence необходимо указать:
    *   `CONFLUENCE_BASE_URL`: Базовый URL вашего экземпляра Confluence (например, `https://wiki.example.com`).
    *   `CONFLUENCE_USERNAME`: Имя пользователя для доступа к Confluence.
    *   `CONFLUENCE_PASSWORD`: Пароль или API-токен для доступа к Confluence.

**Зависимости:**

Убедитесь, что установлены следующие Python пакеты (обычно через `requirements.txt` и `setup.sh`):
*   `requests`
*   `pdfminer.six`
*   `markdownify`
...
# Для страницы Confluence:
python3 convert_pdf_to_req.py "https://wiki.example.com/pages/viewpage.action?pageId=12345"
```
*(Примечание: URL-адреса, содержащие специальные символы, рекомендуется заключать в кавычки)*

**Результаты Выполнения:**
*   Структурированные папки с Markdown файлами и вложениями в директории `create_final_tests/folder_structure/`.
